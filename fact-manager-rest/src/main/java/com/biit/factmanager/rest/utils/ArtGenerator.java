...
public class ArtGenerator {
    ...
    private ArtGenerator(String text, int height) {
        try {
            final ArtGenerator artGenerator = new ArtGenerator();
            artGenerator.printTextArt(text.toUpperCase(), height);
        } catch (Exception e) {
            System.err.println(text);
        }
    }

    public static void print(String text, int height) {
        new ArtGenerator(text, height);
    }
    ...
    public void printTextArt(String artText, int textHeight, ASCIIArtFont fontType, String artSymbol) {
        final String fontName = fontType.getValue();
        final int imageWidth = findImageWidth(textHeight, artText, fontName);

        final BufferedImage image = new BufferedImage(imageWidth, textHeight, BufferedImage.TYPE_INT_RGB);
        final Graphics g = image.getGraphics();
        final Font font = new Font(fontName, Font.BOLD, textHeight);
        g.setFont(font);
    ...
    public void printTextArt(String artText, int textHeight) {
        printTextArt(artText, textHeight, ASCIIArtFont.ART_FONT_SANS_SERIF, DEFAULT_ART_SYMBOL);
    }
    ...
    private int findImageWidth(int textHeight, String artText, String fontName) {
        final BufferedImage im = new BufferedImage(1, 1, BufferedImage.TYPE_INT_RGB);
